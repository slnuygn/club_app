...
The repository contains a focused suite of unit tests that validate the behavior of core service classes without requiring live Firebase services. The unit test file \texttt{test/api\_test\_mockup.dart} instantiates service objects using lightweight fakes and mocks:
...
\begin{itemize}
    ...
    \item To run: \texttt{flutter test test/api\_test\_mockup.dart}.
\end{itemize}
...
\subsection{Real Firebase API Tests (\texttt{test/api\_test\_real.dart})}

For completeness, the repository includes \texttt{test/api\_test\_real.dart}, which attempts to exercise the real Firebase Cloud Firestore and Storage APIs from a test harness. This file mirrors the unit-test scenarios (connectivity checks, CRUD on posts/clubs, likes/follows, query filters, stress/bulk operations) but targets the actual backend instead of fakes.

\paragraph{Platform constraint.} Running \texttt{flutter test} on the Dart VM does not provide a device or platform channels required by Firebase plugins. As a result, real API tests will typically fail with errors such as:
\begin{itemize}
    \item \texttt{[core/no-app] No Firebase App '[DEFAULT]' has been created}
    \item \texttt{PlatformException(channel-error, Unable to establish connection ...)}
\end{itemize}
...
\paragraph{Recommended approach.} We provide a device-based counterpart at \texttt{integration\_test/api\_integration\_test.dart}. It initializes Firebase via \texttt{IntegrationTestWidgetsFlutterBinding} and \texttt{Firebase.initializeApp(options: DefaultFirebaseOptions.currentPlatform)}, then executes the same scenarios against the live backend, cleaning up documents created in \texttt{users}, \texttt{clubs}, and \texttt{posts}.

\subsection{Test README Summary}

The test folder includes a \texttt{README.md} that distinguishes unit vs. integration tests and provides run instructions:
\begin{itemize}
    \item \textbf{Prerequisites:} Flutter SDK; Firebase configured (\texttt{lib/firebase\_options.dart}, Android \texttt{google-services.json}); network connectivity; an Android emulator or device for integration tests.
    \item \textbf{Run unit tests (VM):}
    \begin{verbatim}
    flutter test test
    flutter test test/api_test_mockup.dart
    flutter test test/stress_test.dart
    \end{verbatim}
    \item \textbf{Run real Firebase integration tests (device/emulator):}
    \begin{verbatim}
    flutter pub get
    flutter devices
    flutter test integration_test/api_integration_test.dart -d <deviceId>
    \end{verbatim}
    \item \textbf{Optional emulator:} To avoid touching production data, point Firestore to the emulator in test setup with \texttt{FirebaseFirestore.instance.useFirestoreEmulator(host, port)}.
    \item \textbf{Cleanup and troubleshooting:} Integration tests delete created documents; if a run aborts, leftover test docs can be removed safely. Errors like \texttt{channel-error} or \texttt{no-app} indicate tests were run in the VM instead of on a device; use the integration test.
\end{itemize}
